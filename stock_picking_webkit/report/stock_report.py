...
class PrintPick(report_sxw.rml_parse):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        for agr in agreg:
            ...
            picking_ids.append(agreg[agr][0].picking_id.id)
...
class DeliveryOrder(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        super(DeliveryOrder, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            'invoice_address': self._get_invoice_address,
        })
...
report_sxw.report_sxw('report.webkit.delivery_order',
                      'stock.picking',
                      'addons/stock_picking_webkit/report/delivery_order.mako',
                      parser=DeliveryOrder)
